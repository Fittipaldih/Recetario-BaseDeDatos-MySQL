...
where r.id = (select recetaFavorita.idRecetaFavorita
                from (select max(cantidadMaxima.total), cantidadMaxima.idReceta as idRecetaFavorita
                        from  (select count(ccr.id_receta) as total, ccr.id_receta as idReceta
                                ...
                                having  m.fuePreparado=true) as cantidadMaxima) 
			as recetaFavorita);
...
where exists ( select 1 
		from menu m 
	      	join menu_contiene_comida mcc on mcc.id_comida = m.id 
		where m.id = mcc.id_comida 
                and month(m.fecha_desde) = month(curdate()) 
                and year(m.fecha_desde) = year(curdate()))
		or exists  ( select 1 
			     from menu m 
			     join menu_contiene_comida mcc on mcc.id_comida = m.id 
			     where m.id = mcc.id_comida 
               		     and month(m.fecha_hasta)=month(curdate()) 
                	     and year(m.fecha_hasta) = year(curdate()))
group by m.id;

update menu set valor_inflacionario = 5.0 where id = 1;
...
where m.id in (select marcaFavorita.idMarcaFavorita
			...
			from  (select count(lcii.id_Ingrediente) as total, i.id_marca as idMarca
				from lista_compras_incluye_ingrediente lcii 
			       	join ingrediente i on i.id=lcii.id_Ingrediente
				group by i.id_marca, lcii.comprado_o_no
				having  lcii.comprado_o_no=true) as cantidadMaxima) 	
			as marcaFavorita);
...
where not exists (select 1
		  from comida_es_conjunto_receta ccr2 
		  where ccr2.preparada = false
...
where exists ( select 1 
		from menu m 
	     	join menu_contiene_comida mcc on mcc.id_menu = m.id 
		where m.fecha_desde between date_sub(curdate(), interval 30 day) -- date sub devuelve la diferencia de tipos date. Curdate es la fecha actual
                and curdate() 
                and com.id = mcc.id_comida )
		or exists  ( 	select 1 
				from menu m 
			    	join menu_contiene_comida mcc on mcc.id_menu = m.id 
				where m.fecha_hasta between date_sub(curdate(), interval 30 day) and curdate() 
                		and com.id = mcc.id_comida )
group by c.id;
